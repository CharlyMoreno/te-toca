...
export default function GameHouse({ home, user, members: initialMembers, onLogout, authError }: { home: House; user: Person; members: Person[]; onLogout: () => void; authError: string }) {
  ...
  const [taskPet,setTaskPet]=useState<string|null>(null)
  ...
  const [dialog,setDialog]=useState<'task'|'invite'|'history'|'routines'|'avatar'|'scores'|'menu'|'rooms'|'pets'|null>(null)
  ...
  const [history,setHistory]=useState<{id:string;title:string;action:string;display_name:string;created_at:number;room:RoomId;pet_name:string|null}[]>([])
  ...
  const refresh=useCallback(async()=>{
    ...
    const [data,house,layout,animals]=await Promise.all([request<{occurrences:Occurrence[];routines:Routine[];scores:Score[];today:string}>('/tasks'),request<{members:Person[]}>('/homes/current'),request<{rooms:RoomRecord[]}>('/rooms'),request<{pets:Pet[]}>('/pets')])
    if(version!==refreshVersion.current)return
    const updatedRooms=layoutRooms(layout.rooms)
    setPets(animals.pets)
    setTaskPet(previous=>animals.pets.some(pet=>pet.id===previous)?previous:null)
    setFocus(previous=>animals.pets.some(pet=>pet.id===previous)||house.members.some(person=>person.id===previous)?previous:null)
  ...
  function newTask(id:RoomId=room??rooms[0]?.id,petId:string|null=pets.some(pet=>pet.id===focus)?focus:null) {
    if(!id||!rooms.some(room=>room.id===id)){setError('Todavía estamos cargando los ambientes.');return}
    setTaskPet(petId);setTaskRoom(id);setIcon(petId?'dishes':rooms.find(r=>r.id===id)!.icon);setTitle(petId?`Dar de comer a ${pets.find(pet=>pet.id===petId)?.name??'mi mascota'}`:rooms.find(r=>r.id===id)!.suggestion);setFirstDate(today);setFrequency('daily');setPoints(10);setParticipants(members.map(p=>p.id));setDialog('task')
  }
  function createTask(event:FormEvent) {
    event.preventDefault()
    void act(async()=>{
      const result=await request<{event:GameEvent}>('/tasks','POST',{title,room:taskRoom,icon,frequency,firstDate,participants,points,petId:taskPet})
      receiveEvent(result.event)
      await refresh();setDialog(null);setFocus(taskPet);chooseRoom(taskRoom)
    })
  }
  ...
  function choosePet(id:string){setFocus(id);setRoom(null);setSelected(null);setTab('pending');setPanelOpen(true);setCharacter(null);setDialog(null)}
  const focusedPet=pets.find(pet=>pet.id===focus)
  const currentRoom=rooms.find(r=>r.id===room)
  const currentTask=tasks.find(t=>t.id===selected)
  const inView=tasks.filter(t=>(!room||t.room===room)&&(!focus||t.assignee===focus||t.pet_id===focus))
  ...
  const name=(id:string)=>members.find(p=>p.id===id)?.display_name??pets.find(p=>p.id===id)?.name??'Integrante'
  return <div className="game-shell">
    ...
    <main className={`game-layout immersive ${panelOpen?'panel-open':'panel-closed'}`}>
      ...
      <aside id="house-tasks" className="tasks-sidebar" hidden={!panelOpen}><button className="panel-close" onClick={()=>setPanelOpen(false)} aria-label="Cerrar tareas">×</button>
        <div className="sidebar-title"><span className="game-eyebrow">{focusedPet?'MASCOTA':currentRoom?'ESTÁS EN':'VISTA GENERAL'}</span><h2>{focusedPet?`Las tareas de ${focusedPet.name}`:currentRoom?.name??'Tu casa, al día'}</h2><p>{pending.length} pendientes{focus&&!focusedPet?` · ${name(focus)}`:''}</p>{focus&&<button className="game-link" onClick={()=>setFocus(null)}>Ver a todos</button>}{focusedPet&&<div className="pet-task-context"><button className="game-link" onClick={()=>newTask(room??rooms[0]?.id,focusedPet.id)}>+ Asignarle una tarea</button></div>}</div>
        ...
        <div className="task-list">
          ...
          {visible.map(task=><button key={task.id} className={`task-row ${selected===task.id?'selected':''}`} onClick={()=>chooseTask(task)}><span className={`task-check ${task.completed_at?'done':''}`}>{task.completed_at?'✓':'·'}</span><span><strong>{task.title} <em className="points-badge">+{task.points} pts</em></strong><small>{task.pet_id?`🐾 ${name(task.pet_id)} · `:''}{name(task.assignee)} · {task.due_date===today?'Hoy':task.due_date}{!room&&` · ${rooms.find(r=>r.id===task.room)?.name}`}</small></span><span>↗</span></button>)}
        </div>
        {currentTask&&<section className="task-detail"><button className="detail-close" onClick={()=>setSelected(null)} aria-label="Cerrar detalle">×</button><span className="game-eyebrow">{currentTask.frequency==='daily'?'CADA DÍA':'CADA SEMANA'}</span><h3>{currentTask.title} <em className="points-badge">+{currentTask.points} pts</em></h3><p>{currentTask.pet_id&&<>Para <strong>🐾 {name(currentTask.pet_id)}</strong>. </>}Le toca a <strong>{name(currentTask.assignee)}</strong>{currentTask.pet_id?' acompañarla y registrar la tarea.':'.'}</p>{currentTask.completed_at?<div className="completed-note">✓ Hecha el {new Date(currentTask.completed_at*1000).toLocaleDateString('es-AR')}</div>:currentTask.assignee===user.id&&currentTask.due_date<=today?<><p className="real-task-note">Cuando la termines en casa, registrala acá.</p><button className="game-primary" disabled={busy} onClick={()=>complete(currentTask)}>{busy?'Guardando…':currentTask.pet_id?'Tarea de mi mascota lista ✓':'Ya la hice ✓'}</button></>:<p className="real-task-note">{currentTask.due_date>today?'Este turno es para más adelante.':'Su responsable puede marcarla como hecha.'}</p>}{!currentTask.completed_at&&currentTask.assignee!==user.id&&currentTask.due_date<=today&&<button className="nudge-button" disabled={busy} onClick={()=>nudge(currentTask)}>📣 Retar con onda</button>}{currentTask.nudged_at&&!currentTask.completed_at&&<p className="real-task-note">{name(currentTask.nudged_by!)} recordó esta tarea el {new Date(currentTask.nudged_at*1000).toLocaleString('es-AR',{timeZone:home.timezone})}.</p>}</section>}
    ...
    {dialog==='pets'&&<Dialog title="Mascotas de la casa" close={close}><PetEditor pets={pets} tasks={tasks} today={today} admin={home.role==='admin'} changed={refresh} onTasks={choosePet} onTask={id=>newTask(room??rooms[0]?.id,id)} /></Dialog>}
    ...
    {dialog==='task'&&<Dialog title="Una nueva tarea" close={close}><form onSubmit={createTask} className="task-form"><label>¿Qué hay que hacer?<input autoFocus required minLength={2} maxLength={80} value={title} onChange={e=>setTitle(e.target.value)} /></label><div className="form-columns"><label>Habitación<select value={taskRoom} onChange={e=>setTaskRoom(e.target.value as RoomId)}>{rooms.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></label><label>Se repite<select value={frequency} onChange={e=>setFrequency(e.target.value)}><option value="daily">Cada día</option><option value="weekly">Cada semana</option></select></label></div><label>Asignar a una mascota<select value={taskPet??''} onChange={event=>{setTaskPet(event.target.value||null);if(event.target.value)setIcon('dishes')}}><option value="">Tarea general de la casa</option>{pets.map(pet=><option key={pet.id} value={pet.id}>🐾 {pet.name}</option>)}</select><small>{pets.length?'La mascota tendrá la tarea en su ficha y una persona será responsable de completarla.':'Agregá perros o gatos desde Menú → Mascotas.'}</small></label>{taskPet&&<div className="pet-task-suggestions">{['Dar de comer','Pasear','Practicar un truco'].map(action=><button key={action} type="button" onClick={()=>setTitle(`${action} · ${name(taskPet)}`)}>{action}</button>)}</div>}<label>Objeto en la habitación<select value={icon} onChange={e=>setIcon(e.target.value)}><option value="dishes">{taskPet?'Comedero de mascota':'Platos'}</option><option value="trash">Bolsa de basura</option><option value="clean">Escoba</option><option value="laundry">Canasto de ropa</option></select></label><label>Puntos por completarla<input type="number" required min={5} max={100} step={1} value={points} onChange={e=>setPoints(Number(e.target.value))} /><small>De 5 a 100. Elegí el valor según el esfuerzo.</small></label><label>Primer turno<input type="date" required min={today} value={firstDate} onChange={e=>setFirstDate(e.target.value)} /></label><fieldset><legend>{taskPet?'¿Quién cuida a la mascota?':'¿Entre quiénes rota?'}</legend><p>Elegí una persona para asignársela, o varias para rotar. El número indica el orden.</p>{members.map(person=><label className="participant-option" key={person.id}><input type="checkbox" checked={participants.includes(person.id)} onChange={()=>setParticipants(participants.includes(person.id)?participants.filter(id=>id!==person.id):[...participants,person.id])} /><span>{person.display_name}</span><b>{participants.includes(person.id)?participants.indexOf(person.id)+1:'—'}</b></label>)}</fieldset><p className="rotation-preview">Próximos responsables: {[0,1,2].map(i=>participants.length?name(participants[i%participants.length]):'—').join(' → ')}</p><button className="game-primary" disabled={busy||!participants.length}>{busy?'Creando…':'Agregar a la habitación'}</button></form></Dialog>}
