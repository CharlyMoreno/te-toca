...
tasks.get('/', async c => {
  ...
  const result = await c.env.DB.prepare(`SELECT o.*,t.title,t.room_id room,t.pet_id,t.icon,t.frequency,t.active,n.created_at nudged_at,n.actor_id nudged_by FROM task_occurrences o
  ...
  const routines = await c.env.DB.prepare('SELECT id,title,room_id room,pet_id,active,frequency,points FROM tasks WHERE home_id=? ORDER BY created_at DESC').bind(home.id).all()
...
tasks.post('/', async c => {
  ...
  if (petId !== null && (typeof petId !== 'string' || !await c.env.DB.prepare('SELECT id FROM pets WHERE id=? AND home_id=?').bind(petId,home.id).first())) {
    return c.json({ error: 'Elegí una mascota de esta casa.' }, 400)
  }
  const today = dateInZone(home.timezone)
  ...
  await c.env.DB.batch([
    c.env.DB.prepare('INSERT INTO tasks (id,home_id,title,room,icon,frequency,first_date,created_at,points,room_id,pet_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)')
      .bind(id, home.id, title, selectedRoom.kind, icon, frequency, firstDate, now(), points, room, petId),
...
tasks.get('/history', async c => {
  const home = await membership(c.env.DB, c.get('user').id)
  if (!home) return c.json({ error: 'No tenés una casa.' }, 403)
  const result = await c.env.DB.prepare(`SELECT e.*, t.title, t.room_id room, t.pet_id, p.name pet_name, u.display_name, o.due_date
    FROM activity_events e JOIN task_occurrences o ON o.id=e.occurrence_id
    JOIN tasks t ON t.id=o.task_id JOIN users u ON u.id=e.actor_id LEFT JOIN pets p ON p.id=t.pet_id
